...
# 1. Global Page Layout Setup (Keep the Wide Format and Titled Banner)
...
with tab2:
    ...
    # Target directory folder where Git will push your markdown files from home
    ...
    if not available_briefs:
        ...
        st.caption("Tip: Ensure run_ai_engine.bat has executed successfully at least once on your local terminal machine.")
    else:
        # File selector input box placed natively within Tab 2
        ...
        try:
            ...
            # Clean internal Obsidian links [[ ]] so they display clean for public web layouts
            clean_markdown = re.sub(r'\[\[(.*?)\]\]', r'**\1**', raw_markdown)
            
            # Render the complete localized analysis
            st.markdown("---")
